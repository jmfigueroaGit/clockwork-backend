...
const login = async (req, res) => {
    try {
        ...
        const member = await User.authenticate(username, password);
...
const sendAccountVerificationEmail = async (req, res) => {
    try {
        ...
        // Sending a success response
        res.status(200).json({ message: 'Account verification email sent successfully', verifyToken });
    } catch (error) {
        // Sending an error response
        res.status(500).json({ message: error.message });
    }
...
module.exports = { register, login, me, logout, forgetPassword, resetPassword, refreshToken, sendAccountVerificationEmail, verifyAccount, changePassword };